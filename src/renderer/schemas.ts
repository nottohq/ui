  ...
  .refine(
    (h) => {
      ...
      if (/^https?:\/\//i.test(trimmed)) {
        ...
        const authority = sepIdx === -1 ? afterScheme : afterScheme.slice(0, sepIdx);
...
export const stackPropsSchema = z.strictObject({
  ...
  as: z
    .enum(["div", "section", "main", "header", "footer", "nav", "aside", "ul", "ol", "li", "form"])
    .optional(),
});
...
export const boxPropsSchema = z.strictObject({
  ...
  as: z.enum(["div", "section", "article", "aside", "header", "footer", "main"]).optional(),
});
...
const nottoChildrenSchema: z.ZodType<NottoChildren> = z.lazy(() =>
  z.union([
    ...
    nottoNodeSchema,
    z
      .array(z.union([z.string().max(TEXT_HARD_CAP), z.number(), nottoNodeSchema]))
