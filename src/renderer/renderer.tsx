...
function renderNode(
  ...
  if (ctx.nodeCount.current > ctx.maxNodes) {
    throw new RendererError(`Max node count (${ctx.maxNodes}) exceeded`, "max-nodes", path);
  }

  if (depth > ctx.maxDepth) {
    throw new RendererError(`Max depth (${ctx.maxDepth}) exceeded`, "max-depth", path);
  }

  const entry = PRIMITIVES[node.type];
  if (!entry) {
    throw new RendererError(`Unknown primitive type: "${echo(node.type)}"`, "unknown-type", path);
  }
  ...
  if (typeof props.action === "string") {
    ...
    if (!handler) {
      throw new RendererError(`Unknown action: "${echo(props.action)}"`, "unknown-action", path);
    }
  ...
  if (node.type === "Icon" && typeof props.name === "string") {
    ...
    if (!iconComponent) {
      throw new RendererError(`Unknown icon: "${echo(props.name)}"`, "unknown-icon", path);
    }
...
function checkTextLength(text: string, ctx: RenderContext, path: readonly string[]): void {
